...
def req(path: str, method: str = "GET", json=None, params=None):
    try:
        ...
        r = requests.request(method, url, headers=headers, json=json, params=params, timeout=10)
        return r
...
with st.sidebar:
    ...
    if st.button("Login", key="login_btn"):
        try:
            r = requests.post(API+"/auth/login", data={"username": u, "password": p}, timeout=10)
            if r.ok:
...
with col1:
    ...
    if st.button("Explain", key="nlp_explain"):
        ...
        if r: st.code(r.text, language="json")
    ...
    if st.button("Statistics", key="logs_stats"):
        r = req("/logs/statistics", params={"source": src or None, "start_time": start_t or None, "end_time": end_t or None})
        if r: st.code(r.text, language="json")
    if st.button("Anomalies", key="logs_anoms"):
        r = req("/logs/anomalies", params={"source": src or None, "start_time": start_t or None, "end_time": end_t or None, "interval":"hour", "threshold":2.0})
        if r: st.code(r.text, language="json")
    ...
    if st.button("Digest", key="logs_digest"):
        r = req("/logs/digest", params={"source": src or None, "start_time": start_t or None, "end_time": end_t or None})
        if r: st.code(r.text, language="json")
    ...
    if st.button("Ingest Upload", key="logs_upload") and up:
        ...
        payload = {"source": source, "logs": [{"timestamp": "", "level": "INFO", "message": l} for l in lines]}
...
with col2:
    ...
    if st.button("Analyze Pipeline", key="cicd_analyze"):
        r = req(f"/cicd/pipelines/{pipeline_id}/analysis?token=" + st.session_state.token)
        if r: st.code(r.text, language="json")
    ...
    if st.button("Optimize", key="res_optimize"):
        r = req("/resources/optimize", method="POST", json={"targets":["cpu","storage"]})
        if r: st.code(r.text, language="json")
...
if st.button("Send Notification", key="notif_send"):
    r = req("/notifications/send", method="POST", json={"channels":["email"], "message": lmsg, "recipients":[email]})
    if r: st.code(r.text, language="json")
...
if colp1.button("Forecast (tiny)", key="pred_forecast_btn2"):
    ...
    r = req("/predictions/forecast", method="POST", json={"data": series, "metric_name":"demo", "days":1})
    if r: st.code(r.text, language="json")

colp5, colp6 = st.columns(2)
if colp5.button("Train Model", key="pred_train_btn"):
    dataset = [{"ds": f"2024-01-0{i+1}", "y": i} for i in range(14)]
    r = req("/predictions/train", method="POST", json={"data": dataset, "model_name": "prophet"})
    if r: st.code(r.text, language="json")
if colp6.button("Batch Predict", key="pred_batch_btn"):
    batch = {
        "predictions": [
            {"type": "forecast", "data": {"data": [{"ds": f"2024-01-0{i+1}", "y": i} for i in range(7)], "metric_name": "demo", "days": 1}},
            {"type": "anomalies", "data": {"data": [{"ds": f"2024-01-0{i+1}", "y": i} for i in range(7)], "metric_name": "demo", "threshold": 2.0}}
        ]
    }
...
if colp2.button("Anomalies", key="pred_anomalies_btn"):
    series = [{"ds": f"2024-01-0{i+1}", "y": i} for i in range(7)]
    r = req("/predictions/anomalies", method="POST", json={"data": series, "metric_name":"demo", "threshold": 2.0})
    if r: st.code(r.text, language="json")
if colp3.button("Predict Resources", key="pred_resources_btn"):
    hist = [{"t": i, "cpu": i%5} for i in range(24)]
    r = req("/predictions/resources/predict", method="POST", json={"data": hist, "resource_type": "cpu", "hours": 24})
    if r: st.code(r.text, language="json")
if colp4.button("Predict Incidents", key="pred_incidents_btn"):
    ...
    r = req("/predictions/incidents/predict", method="POST", json={"historical_incidents": hist, "system_metrics": metrics})
    if r: st.code(r.text, language="json")


if cols[1].button("Generate Report", key="rep_generate"):
    r = req("/reports/generate", method="POST", json={"template_id":"demo","parameters":{},"format":"json"})
    if r: st.code(r.text, language="json")
...
cells = st.columns(5)
services = ["users","nlp","logs","predictions","monitoring","cicd","resources","notifications","reports"]
for i, svc in enumerate(services):
    if cells[i % 5].button(f"{svc}", key=f"health_{svc}"):
        r = req(f"/{svc}/health")
        if r: st.code(r.text, language="json")

st.caption("Set GATEWAY_BASE env var to target a different gateway host (default http://localhost:8080)")
