...
async def search_log_data(
    ...
    if result.get("status") == "error":
        raise HTTPException(status_code=_http_error_for(result), detail=result.get("message"))
...
async def get_log_stats(
    ...
    # Generate cache key
    cache_key = f"log_stats:{source or 'all'}:{start_time or ''}:{end_time or ''}:{interval}"
...
async def log_digest(
    ...
    if stats.get("status") == "error":
        raise HTTPException(status_code=_http_error_for(stats), detail=stats.get("message"))
    ...
    if recent_errors.get("status") == "error":
        ...
        recent_list = [
            {"timestamp": l.get("timestamp"), "level": l.get("level"), "message": l.get("message", "")[:300]}
            for l in recent_errors.get("logs", [])
        ]
    ...
    return {
        "status": "success",
        "summary": {
            "time_range": {"start": stats.get("start_time"), "end": stats.get("end_time")},
            "total": sum([p.get("count", 0) for p in stats.get("time_series", [])]),
            "errors": sum([p.get("error_count", 0) for p in stats.get("time_series", [])]),
...
async def admin_delete_by_query(
    ...
    try:
        ...
        if st or et:
            rng = {"range": {"timestamp": {}}}
            if st: rng["range"]["timestamp"]["gte"] = st
            if et: rng["range"]["timestamp"]["lte"] = et
            filters.append(rng)
        dq = {"query": {"bool": {"must": must, "filter": filters}}}
        indices = f"{settings.LOG_INDEX_PREFIX}*" if not source else f"{settings.LOG_INDEX_PREFIX}{source}-*"
        resp = es_client.delete_by_query(index=indices, body=dq, conflicts="proceed", refresh=True, slices="auto")
        return {"status": "success", "deleted": resp.get("deleted", 0), "took": resp.get("took")}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))