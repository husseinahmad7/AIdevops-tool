...
async def prometheus_metrics():
    ...
    if settings.DOCKER_ENABLED:
        ...
        running_containers = sum(
            1 for container in docker_metrics if container["status"] == "running"
        )
...
async def get_all_metrics(request: Request):
    ...
    result = {"system": system_metrics, "timestamp": datetime.now().isoformat()}
...
async def get_resource_history(request: Request, resource_type: str, days: int = 1):
    user = await verify_token(request)
...
async def update_settings(request: Request, data: Dict[str, Any] = Body(...)):
    ...
    if "monitoring_interval" in data:
        ...
        if interval < 10 or interval > 3600:
            raise HTTPException(
                status_code=400, detail="Interval must be between 10 and 3600 seconds"
            )
    ...
    if "cpu_threshold" in data:
        ...
        if threshold < 0 or threshold > 100:
            raise HTTPException(
                status_code=400, detail="CPU threshold must be between 0 and 100"
            )
    ...
    if "memory_threshold" in data:
        ...
        if threshold < 0 or threshold > 100:
            raise HTTPException(
                status_code=400, detail="Memory threshold must be between 0 and 100"
            )
    ...
    if "disk_threshold" in data:
        ...
        if threshold < 0 or threshold > 100:
            raise HTTPException(
                status_code=400, detail="Disk threshold must be between 0 and 100"
            )
