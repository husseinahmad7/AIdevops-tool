...
def get_api_keys_by_user(db: Session, user_id: uuid.UUID, skip: int = 0, limit: int = 100):
    return db.query(models.APIKey).filter(models.APIKey.user_id == user_id).offset(skip).limit(limit).all()
