...
async def send_notification(
    notification_request: Dict[str, Any], user: dict = Depends(verify_token)
):
...
async def send_templated_notification(
    template_id: str, template_data: Dict[str, Any], user: dict = Depends(verify_token)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to send templated notification: {str(e)}"
        )


@router.get("/history")
async def get_notification_history(
    limit: int = 50, offset: int = 0, user: dict = Depends(verify_token)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get notification history: {str(e)}"
        )


@router.post("/channels/test")
async def test_notification_channel(
    channel_config: Dict[str, Any], user: dict = Depends(verify_token)
):
...
async def get_notification_stats(user: dict = Depends(verify_token)):
    ...
    try:
        stats = {
            ...
            "recent_activity": {"last_24h": 45, "last_7d": 312, "last_30d": 1247},
        }
