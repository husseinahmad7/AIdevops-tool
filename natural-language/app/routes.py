...
async def generate_infrastructure_code(
    ...
    return {"code": code, "platform": platform, "user_id": user.get("id")}
...
async def explain_devops_concept(concept: str, request: Request):
    ...
    return {"concept": concept, "explanation": explanation, "user_id": user.get("id")}
...
async def search_document_database(request: Request, query: str, limit: int = 5):
    ...
    return {"results": results, "query": query, "user_id": user.get("id")}
