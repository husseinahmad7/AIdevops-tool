...
async def get_resource_usage(
    authorization: Optional[str] = Header(None), user: dict = Depends(verify_token)
):
    """Get current resource usage metrics (real host metrics via infrastructure-monitor)"""
    try:
        headers = {"Authorization": authorization} if authorization else None
        resp = requests.get(
            f"{settings.infrastructure_monitor_url}/api/v1/monitoring/metrics",
            headers=headers,
            timeout=5,
        )
        if resp.status_code != 200:
            raise HTTPException(
                status_code=resp.status_code, detail="Infra monitor unavailable"
            )
        metrics = resp.json()
        ...
        usage_data = {
            ...
            "cpu": {
                ...
                "used_cores": round(
                    (metrics.get("cpu", {}).get("percent", 0) / 100.0)
                    * max(metrics.get("cpu", {}).get("cores", 1), 1),
                    2,
                ),
            ...
            "memory": {
                "total_gb": round(
                    metrics.get("memory", {}).get("total", 0) / (1024**3), 2
                ),
                "used_gb": round(
                    (
                        metrics.get("memory", {}).get("total", 0)
                        - metrics.get("memory", {}).get("available", 0)
                    )
                    / (1024**3),
                    2,
                ),
            ...
            "storage": {
                "total_gb": round(
                    metrics.get("disk", {}).get("total", 0) / (1024**3), 2
                ),
            ...
            "network": {
                "ingress_mbps": round(
                    metrics.get("network", {}).get("bytes_recv", 0) * 8 / 1_000_000, 2
                ),
                "egress_mbps": round(
                    metrics.get("network", {}).get("bytes_sent", 0) * 8 / 1_000_000, 2
                ),
            },
        }
...
async def get_cost_analysis(
    ...
    try:
        ...
        if usage_resp.status_code != 200:
            raise HTTPException(
                status_code=usage_resp.status_code, detail="Infra monitor unavailable"
            )
...
async def optimize_resources(
    ...
    try:
        ...
        if metrics_resp.status_code != 200:
            raise HTTPException(
                status_code=metrics_resp.status_code, detail="Infra monitor unavailable"
            )
        m = metrics_resp.json()
            ...
            recs.append(
                {
                    "type": "scale_out",
                    "resource": "compute",
                    "recommendation": "Add more CPU capacity",
                    "impact": "performance",
                    "confidence": 0.9,
                }
            )
            ...
            recs.append(
                {
                    "type": "memory_tuning",
                    "resource": "memory",
                    "recommendation": "Increase memory or tune usage",
                    "impact": "stability",
                    "confidence": 0.88,
                }
            )
            ...
            recs.append(
                {
                    ...
                    "resource": "storage",
                    "recommendation": "Clean up unused data or expand storage",
                    "impact": "availability",
                    "confidence": 0.82,
                }
            )
            ...
            recs.append(
                {
                    "type": "opt_ok",
                    "resource": "all",
                    "recommendation": "Resources are within optimal ranges",
                    "impact": "none",
                    "confidence": 0.7,
                }
            )
...
async def get_optimization_metrics(user: dict = Depends(verify_token)):
    ...
    try:
        ...
        disk = m.get("disk", {}).get("percent", 0)
        eff = max(
            0, 100 - ((cpu + mem + disk) / 3)
        )  # simplistic inverse of utilization
        metrics = {
            ...
            "resource_utilization": {"cpu": cpu, "memory": mem, "storage": disk},
            "waste_indicators": {
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get optimization metrics: {str(e)}"
        )


@router.post("/alerts")
async def create_resource_alert(
    alert_config: Dict[str, Any], user: dict = Depends(verify_token)
):
